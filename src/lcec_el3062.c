...
typedef struct {
  lcec_el3062_chan_t chans[LCEC_EL3062_CHANS];
} lcec_el3062_data_t;


static ec_pdo_entry_info_t lcec_el3062_channel1[] = {
...
static ec_pdo_entry_info_t lcec_el3062_channel2[] = {
...
static ec_pdo_info_t lcec_el3062_pdos_in[] = {
    {0x1A00, 10, lcec_el3062_channel1},
    {0x1A02, 10, lcec_el3062_channel2}
};

static ec_sync_info_t lcec_el3062_syncs[] = {
    {0, EC_DIR_OUTPUT, 0, NULL},
    {1, EC_DIR_INPUT,  0, NULL},
    {2, EC_DIR_OUTPUT, 0, NULL},
    {3, EC_DIR_INPUT,  2, lcec_el3062_pdos_in},
    {0xff}
};

void lcec_el3062_read(struct lcec_slave *slave, long period);

int lcec_el3062_init(int comp_id, struct lcec_slave *slave, ec_pdo_entry_reg_t *pdo_entry_regs) {
  lcec_master_t *master = slave->master;
  lcec_el3062_data_t *hal_data;
  lcec_el3062_chan_t *chan;
  int i;
  int err;

  // initialize callbacks
  slave->proc_read = lcec_el3062_read;

  // alloc hal memory
  if ((hal_data = hal_malloc(sizeof(lcec_el3062_data_t))) == NULL) {
    rtapi_print_msg(RTAPI_MSG_ERR, LCEC_MSG_PFX "hal_malloc() for slave %s.%s failed\n", master->name, slave->name);
    return -EIO;
  }
  memset(hal_data, 0, sizeof(lcec_el3062_data_t));
  slave->hal_data = hal_data;

  // initializer sync info
  slave->sync_info = lcec_el3062_syncs;

  // initialize pins
  for (i=0; i<LCEC_EL3062_CHANS; i++) {
...
void lcec_el3062_read(struct lcec_slave *slave, long period) {
  lcec_master_t *master = slave->master;
  lcec_el3062_data_t *hal_data = (lcec_el3062_data_t *) slave->hal_data;
